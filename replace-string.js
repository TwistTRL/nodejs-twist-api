 ...
 * @Last Modified time: 2020-02-11 13:05:09
 */
  ...
  fs.access(docsDir, (err) => {
    ...
    fs.access(filesDir, (err) => {
      ...
      fs.copyFile(MED_CAT_XLSX_PATH, NEW_PATH_MED_CAT, (err) => {
        ...
        let datatable_array = MED_CAT_JSON.map(item => [item.RXCUI, item.MEDICATION, item["TWIST CLASS"], item.WITHIN_CLASS_DISPLAY_HIERARCHY, item.CONDITION, item.DRUG_CLASS, item.DRUG_CLASS_MoA, item.DRUG_CLASS_THERAPEUTIC]);
        let datatable_json = {"data":datatable_array};
