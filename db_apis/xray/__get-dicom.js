/*
 * @Author: Peng Zeng 
 * @Date: 2021-01-04 21:27:56 
 * @Last Modified by: Peng Zeng
 * @Last Modified time: 2021-01-04 22:21:09
 */

const SYNAPSE_SERVER = "http://10.7.46.53:7001/images/";
const TRLDSC2_SERVER = "http://10.7.46.137:7001/dicom/";
...
const getPersonDicomList = async (mrn) => {
  ...
  // return ret;
    ...
    await fetch(TRLDSC2_SERVER + "person/index", {
    headers: { Authorization: process.env.SYNAPSE_API_TOKEN },
  })
    .then((res) => res.json())
    .then((json) => (mrn_dict = json.mrn_dict));
    return mrn_dict;
};

const getDicomJpgById = async (dicom_id) => {
  let data;
  console.log("dicom_id :>> ", dicom_id);
  // if (image_id === "test") {
  //   return REACT_BASE64;
  // }
  await fetch(TRLDSC2_SERVER + "download/" + dicom_id, {
    headers: { Authorization: process.env.SYNAPSE_API_TOKEN },
  })
    .then((res) => res.text())
    .then((json) => (data = json));
