 ...
 * @Last Modified time: 2021-03-02 09:17:38
 */
...
const subscribeVitals = async (person_id, io) => {
  const myCallback = (message) => {
    io.to(person_id).emit("currentVitals", message);
  ...
  const options = {
    ...
    binds: { person_id },
    timeout: 120, // Stop after 60 seconds
    // ipAddress: '127.0.0.1',
  ...
  try {
    ...
    // user twist already has this grant
    // const sql = `GRANT CHANGE NOTIFICATION TO ${dbConfig.user}`;
    // await connection.execute(sql);
    console.log("after connection");
    await connection.subscribe(`vitals_${person_id}`, options);
...
const unsubscribeVitals = async (person_id) => {
  let connection;
  try {
    connection = await oracledb.getConnection(dbConfig.poolAlias);
    await connection.unsubscribe(`vitals_${person_id}`);
    console.log(`Unsubscription for ${person_id}`);
  } catch (err) {
    console.error(err);
  } finally {
    if (connection) {
