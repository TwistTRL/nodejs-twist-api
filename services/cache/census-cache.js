 ...
 * @Last Modified time: 2021-02-01 21:41:02
 */
...
INSERT INTO API_CACHE_CENSUS_INFUSION
...
DELETE FROM API_CACHE_CENSUS_INFUSION
WHERE DTUNIX <= ${ts}
`;
...
const insertCensusCache = async (dtunix, current_census_data) => {
  ...
  current_census_data.forEach((element) => {
    ...
    const current_infusions_list = element.INFUSIONS.map((item) => ({
      person_id: element.PERSON_ID,
      dtunix,
    ...
    infusions_binds = [...infusions_binds, ...current_infusions_list];
  });

  // console.log("census_binds :>> ", census_binds);
  // console.log('personnel_binds :>> ', personnel_binds);
  // console.log("infusions_binds :>> ", infusions_binds);

  console.time("insert-database-census");
  const conn = await oracledb.getConnection(dbConfig.poolAlias);
  const insertCensusTable = census_binds.length
    ? await conn.executeMany(INSERT_CENSUS_CACHE_SQL, census_binds)
    : { rowsAffected: 0 };
  console.log("insertCensusTable  :>> ", insertCensusTable);

  const insertPersonnelTable = personnel_binds.length
    ? await conn.executeMany(INSERT_CENSUS_PERSONNEL_CACHE_SQL, personnel_binds)
    : { rowsAffected: 0 };
  console.log("insertPersonnelTable  :>> ", insertPersonnelTable);

  const insertInfusionsTable = infusions_binds.length
    ? await conn.executeMany(INSERT_CENSUS_INFUSIONS_CACHE_SQL, infusions_binds)
    : { rowsAffected: 0 };
