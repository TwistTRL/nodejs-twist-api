...
const initialize = async () => {
  ...
  // every 3 minutes
  const updateCensusSchedule = schedule.scheduleJob(
    { start: startTime, rule: "*/3 * * * *" },
    async () => {
      try {
  ...
  // every day at 1 am
  const deleteExpiredCensusSchedule = schedule.scheduleJob(
    { start: startTime, rule: "0 1 * * *" },
    async () => {
