 ...
 * @Last Modified time: 2021-03-08 10:20:31
 */
...
WITH 
    ...
    PATIENT_ECMO AS (
        SELECT PERSON_ID, ECMO_FLOW_NORM, ECMO_VAD_SCORE, EVENT_END_UNIX
        FROM PATIENT_BED
        JOIN ECMO_VAD USING (PERSON_ID)    
        WHERE :timestamp >= EVENT_END_UNIX
    ),
    
    PATIENT_LATEST_ECMO AS (
      SELECT PERSON_ID, ECMO_FLOW_NORM, ECMO_VAD_SCORE, EVENT_END_UNIX
      FROM (
        SELECT PERSON_ID, ECMO_FLOW_NORM, ECMO_VAD_SCORE, EVENT_END_UNIX, ROW_NUMBER () 
        OVER (PARTITION BY PERSON_ID ORDER BY EVENT_END_UNIX DESC) LATEST_ECMO
        FROM PATIENT_ECMO
      )
      WHERE LATEST_ECMO = 1
    ),
